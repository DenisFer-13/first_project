...
export class UsersController {
  ...
  getUsers(): Promise<User[]> {              //Como es TS, aquí después del método podemos indicar que tipo de dato estamos devolviendo. Como del otro lado viene una promesa...
    
    return this.usersService.getUsers();
  }

  @Get('/usuario/:id')                  //Ejemplo realizando un pedido de un usuario en específico.
  getUser(@Param('id') id: string): Promise<User> {              //Tenemos que invocar al decorador Param para indicar cual será el param a buscar.
  ...
  //Ruta de creación de un usuario en la base de datos.
  @Post('/usuario')
  createUser(@Body() user: CreateUser): Promise<User> {          //Con @Body hacemos que se detecte lo que le estamos mandando por body desde el request.
                                                          //Usamos los DTO para indicarle que datos va a recibir en este caso: user.
    return this.usersService.createUser(user)                   //Definimos esos datos en otra carpeta y archivo, y luego los importamos y utilizamos. 
  }
